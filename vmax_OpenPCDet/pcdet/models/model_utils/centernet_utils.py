...
def decode_bbox_from_pred_dicts(pred_dict, point_cloud_range=None, voxel_size=None, feature_map_stride=None):
    ...
    # Clamp 'dim' before exp to prevent overflow
    dim_raw = pred_dict['dim'].permute(0, 2, 3, 1).contiguous()
    dim_raw_clamped = torch.clamp(dim_raw, min=-10, max=10)
    batch_dim = dim_raw_clamped.exp().view(batch_size, H * W, 3)

    # Compute angle from 'rot' (cos, sin)
    rot_raw = pred_dict['rot'].permute(0, 2, 3, 1).contiguous().view(batch_size, H * W, 2)
    batch_rot_cos = rot_raw[..., 0:1]
    batch_rot_sin = rot_raw[..., 1:2]
    angle = torch.atan2(batch_rot_sin, batch_rot_cos)

    # Handle velocity if present
    if 'vel' in pred_dict:
        batch_vel = pred_dict['vel'].permute(0, 2, 3, 1).contiguous().view(batch_size, H * W, 2)
    else:
        batch_vel = None
    ...
    # Scale and shift to absolute coordinates
    xs = xs * feature_map_stride * voxel_size[0] + point_cloud_range[0]
    ys = ys * feature_map_stride * voxel_size[1] + point_cloud_range[1]
    ...
    # Concatenate final predictions into (B, H, W, 7 or 9)
